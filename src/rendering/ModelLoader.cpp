...
std::unordered_map<unsigned int, std::shared_ptr<Mesh>> ModelLoader::meshCache;

bool ModelLoader::LoadModel(const std::string& path, VulkanDevice& device, std::vector<ModelInstance>& outInstances)
{
...
void ModelLoader::ProcessNode(aiNode* node, const aiScene* scene, const glm::mat4& parentTransform, VulkanDevice& device, std::vector<ModelInstance>& outInstances)
{
    ...
    for (unsigned int i = 0; i < node->mNumMeshes; ++i) {
        ...
        //if (mesh->mNumVertices < 50 || mesh->mNumFaces < 10) continue;

        std::shared_ptr<Mesh> gpuMesh;

        if (meshCache.count(meshIndex)) {
            gpuMesh = meshCache[meshIndex];
        }
        else {
            gpuMesh = ProcessMesh(mesh, device);
            if (!gpuMesh) continue;
            meshCache[meshIndex] = gpuMesh;
        }
...
std::shared_ptr<Mesh> ModelLoader::ProcessMesh(const aiMesh* mesh, VulkanDevice& device) {
    ...
    // Create dummy buffer to get proper memoryTypeBits
    VkBufferCreateInfo tempBufferInfo{};
    tempBufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    tempBufferInfo.size = sizeof(Vertex); // just a small dummy
    tempBufferInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    tempBufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

    VkBuffer tempBuffer;
    vkCreateBuffer(logicalDevice, &tempBufferInfo, nullptr, &tempBuffer);

    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(logicalDevice, tempBuffer, &memRequirements);

    uint32_t memoryTypeIndex = device.FindMemoryType(
        memRequirements.memoryTypeBits,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
    );

    // Cleanup dummy buffer
    vkDestroyBuffer(logicalDevice, tempBuffer, nullptr);

    return std::make_shared<Mesh>(logicalDevice, memoryTypeIndex, vertices, indices);
}
